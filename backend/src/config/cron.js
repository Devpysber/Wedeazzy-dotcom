...
function initCron() {
  ...
  // 9. WhatsApp Failed/Offline Message Retry Sweep (every 5 minutes)
  // Was documented in whatsapp.service.js as running on this schedule but was
  // never actually wired to a cron job — messages that failed while Baileys
  // was offline stayed at status "queued" forever with no automatic retry.
  cron.schedule('*/5 * * * *', async () => {
    try {
      const result = await whatsappService.retryFailedMessages();
      if (result && result.retried) {
        logger.info(result, '[CRON] WhatsApp retry sweep completed');
      }
    } catch (err) {
      logger.error({ err }, '[CRON] WhatsApp retry sweep failed');
    }
  });

  // 10. Stuck Payment Reconciliation (every 10 minutes)
