...
async function requireAuth(req, res, next) {
  try {
    ...
    // 4. Admin-issued credentials must be changed before any other authenticated
    // action is allowed — enforced here (not just client-side) so a stored token
    // can't be used to skip the forced password-change screen.
    if (user.mustChangePassword) {
      const path = req.originalUrl.split('?')[0];
      const allowedWhileMustChange = ['/api/auth/change-password', '/api/auth/me', '/api/auth/user', '/api/auth/logout'];
      if (!allowedWhileMustChange.includes(path)) {
        throw new HttpError(403, 'You must set a new password before continuing.', 'ERR_MUST_CHANGE_PASSWORD');
      }
    }
