 ...
 * `image` is whatever Baileys' `image` field accepts: a Buffer, or
 * `{ url: 'https://...' }` for a remote/publicly-reachable URL.
 * Throws with `err.code === 'WA_OFFLINE'` when not connected.
 */
async function sendImage(toE164, image, caption) {
  if (!sock || status !== 'online') {
    const err = new Error(`WhatsApp not online (status=${status})`);
    err.code = 'WA_OFFLINE';
    throw err;
  ...
  const jid = jidFor(toE164);
  await sock.sendMessage(jid, { image, caption: caption || undefined });
}
