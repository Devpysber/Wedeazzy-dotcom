 ...
 * @param {string}  opts.body     - Message text (used as the image caption when mediaUrl is set)
...
async function sendWa({ to, body, template, campaignName = null, userId = null, existingId = null, mediaUrl = null }) {
  ...
  if (existingId) {
    ...
    log = await prisma.waMessage.create({
      data: { to: phone, body, template, campaignName, userId, status: 'queued' },
    });
  }
  ...
  try {
    if (mediaUrl) {
      await baileys.sendImage(phone, { url: mediaUrl }, body);
    } else {
