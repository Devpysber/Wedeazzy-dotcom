...
async function upsertProfile(user, patch) {
  if (!patch) patch = {};
  const data = {
    partnerName: patch.partnerName != null ? sanitizeText(String(patch.partnerName), 200) : undefined,
    weddingDate: patch.weddingDate ? new Date(patch.weddingDate) : undefined,
    city: patch.city != null ? sanitizeText(String(patch.city), 100) : undefined,
    ...
    vibe: patch.vibe != null ? sanitizeText(String(patch.vibe), 200) : undefined,
    notes: patch.notes != null ? sanitizeText(String(patch.notes), 2000) : undefined,
  };
...
async function getMyDashboard(user) {
  ...
  const [shortlists, inquiries, planTasks, bookings, guests] = await Promise.all([
  ...
  return { couple, shortlists: fattenedShortlists, inquiries, planTasks, bookings, guests };
}
...
async function createTask(user, patch) {
  ...
  return prisma.planTask.create({
    data: {
      coupleId: couple.id,
      title: sanitizeText(patch.title, 190),
...
const VALID_RSVP = ['Attending', 'Pending', 'Declined'];

async function createGuest(user, patch) {
  const couple = await getCoupleOrThrow(user.id, 'No couple profile found');

  if (!patch.name || !String(patch.name).trim()) throw new HttpError(400, 'Guest name is required', 'ERR_INPUT');
  const count = Math.max(1, parseInt(patch.count, 10) || 1);
  const rsvp = VALID_RSVP.includes(patch.rsvp) ? patch.rsvp : 'Pending';

  return prisma.guest.create({
    data: {
      coupleId: couple.id,
      name: sanitizeText(patch.name, 190),
      count,
      rsvp,
      category: patch.category ? sanitizeText(patch.category, 100) : 'Friends',
    }
  });
}

async function updateGuest(user, guestId, patch) {
  const couple = await getCoupleOrThrow(user.id, 'No couple profile found');

  const guest = await prisma.guest.findFirst({ where: { id: guestId, coupleId: couple.id } });
  if (!guest) throw new HttpError(404, 'Guest not found', 'ERR_NO_GUEST');

  const data = {};
  if (patch.rsvp !== undefined && VALID_RSVP.includes(patch.rsvp)) data.rsvp = patch.rsvp;
  if (patch.count !== undefined) data.count = Math.max(1, parseInt(patch.count, 10) || 1);
  if (patch.name !== undefined && String(patch.name).trim()) data.name = sanitizeText(patch.name, 190);

  return prisma.guest.update({ where: { id: guestId }, data });
}

async function deleteGuest(user, guestId) {
  const couple = await getCoupleOrThrow(user.id, 'No couple profile found');
