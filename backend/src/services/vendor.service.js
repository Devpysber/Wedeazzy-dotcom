...
async function signupOrAttach(user, payload) {
  ...
  const businessName = sanitizeText(String(payload.businessName || ''), 150);
  const category = sanitizeText(String(payload.category || ''), 100);
  const city = sanitizeText(String(payload.city || ''), 100);
...
async function updateProfile(user, patch, vendorId = null) {
  ...
  if (patch.name !== undefined || patch.email !== undefined) {
    const userData = {};
    if (patch.name !== undefined) userData.name = sanitizeText(patch.name.trim(), 190);
  ...
  sanitizeFields(data, [
    'businessName', 'category', 'city', 'area', 'address', 'pincode', 'googleCid',
    'description', 'whatsappNumber', 'alternateMobile', 'website', 'instagram',
    'facebook', 'youtube', 'businessTimings', 'serviceAreas', 'languagesSpoken', 'googleBusiness',
  ], data.description !== undefined ? 2000 : 500);
  if (data.businessName !== undefined) data.businessName = sanitizeText(data.businessName, 190);
...
async function setCoverPhoto(user, photoId, vendorId = null) {
  ...
  // Ownership check — without this, any vendor could set another vendor's
  // photo as their own cover just by guessing/reusing a photoId (IDOR).
  const target = await prisma.vendorPhoto.findUnique({ where: { id: photoId } });
  if (!target || target.vendorId !== v.id) throw new HttpError(404, 'Photo not found', 'ERR_NO_PHOTO');
