...
async function bulkImportVendors(vendors) {
  ...
  for (let idx = 0; idx < vendors.length; idx++) {
    ...
    try {
      // Bulk import is the origin of the platform's ~13,000-row vendor
      // dataset — an unsanitized poisoned row here becomes stored XSS on
      // public listing/detail pages, unlike hand-entered admin forms which
      // already sanitize.
      const name = sanitizeText(String(row.businessName || ''), 150);
      const category = sanitizeText(String(row.category || ''), 100);
      const city = sanitizeText(String(row.city || ''), 100);
      ...
      const vendorData = {
        ...
        city,
        citySlug,
        area: row.area ? sanitizeText(String(row.area), 100) : null,
        address: row.address ? sanitizeText(String(row.address), 500) : null,
        pincode: row.pincode ? String(row.pincode).trim().slice(0, 10) : null,
...
async function bulkImportUsers(users) {
  ...
  for (let idx = 0; idx < users.length; idx++) {
    ...
    try {
      ...
      const name = row.name ? sanitizeText(String(row.name), 150) : null;
      ...
      if (role === 'couple') {
        ...
        const coupleData = {
          partnerName: row.partnerName ? sanitizeText(String(row.partnerName), 200) : null,
          weddingDate,
          city: row.city ? sanitizeText(String(row.city), 100) : null,
          ...
          vibe: row.vibe ? sanitizeText(String(row.vibe), 200) : null,
          notes: row.notes ? sanitizeText(String(row.notes), 2000) : null
        };
