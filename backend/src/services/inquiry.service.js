...
async function listForVendor(vendorUserId, { status, limit = 50 } = {}, isAdmin = false) {
  ...
  // userId is not a unique column on Vendor (a user can theoretically own more
  // than one listing) — findUnique here would throw on every call. findFirst
  // matches the pattern used elsewhere (e.g. campaign.controller.js).
