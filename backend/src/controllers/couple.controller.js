...
async function createGuest(req, res, next) {
  try {
    const guest = await service.createGuest(req.user, req.body || {});
...
async function updateGuest(req, res, next) {
  try {
    const guest = await service.updateGuest(req.user, req.params.guestId, req.body || {});
    res.json({ ok: true, guest });
  } catch (e) { next(e); }
}
...
async function deleteGuest(req, res, next) {
  try {
    const r = await service.deleteGuest(req.user, req.params.guestId);
    res.json(r);
  } catch (e) { next(e); }
}
