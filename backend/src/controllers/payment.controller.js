...
async function verifyPayment(req, res, next) {
  try {
    ...
    // Ownership + order-id binding checks. Without these, a valid signature
    // for the caller's OWN real payment could be replayed against a
    // DIFFERENT transaction id (their own abandoned expensive checkout, or
    // in principle another user's transaction if a txnId ever leaked) —
    // verifyRazorpaySignature only proves the (order_id, payment_id,
    // signature) triple is internally consistent, not that it belongs to
    // THIS transaction. activateSubscription would then activate whatever
    // plan/campaign that other transaction represents using this payment's
    // proof: pay for a cheap plan, get the expensive one activated.
    const txn = await prisma.transaction.findUnique({ where: { id: transactionId } });
    if (!txn) {
      throw new HttpError(404, 'Transaction not found', 'ERR_NOT_FOUND');
    }
    if (txn.userId !== req.user.id) {
      throw new HttpError(403, 'This transaction does not belong to your account', 'ERR_FORBIDDEN');
    }
    const expectedOrderId = txn.meta && typeof txn.meta === 'object' ? txn.meta.razorpayOrderId : null;
    if (!expectedOrderId || expectedOrderId !== razorpay_order_id) {
      logger.error({ transactionId, razorpay_order_id, expectedOrderId }, 'Razorpay order id does not match the transaction being verified');
      throw new HttpError(400, 'Payment does not match this transaction.', 'ERR_ORDER_MISMATCH');
    }
    ...
    const planName = updatedTxn?.purpose?.startsWith('subscription:') ? updatedTxn.purpose.slice(13) : 'Ad Campaign';

    res.json({
      ok: true,
      message: 'Payment verified and subscription activated successfully.',
      plan: planName,
      amount: updatedTxn?.amount ? (updatedTxn.amount / 100).toFixed(2) : '0',
      transactionId,
