...
async function signup(req, res, next) {
  try {
    const body = sanitizeFields(req.body || {}, ['name', 'businessName', 'category', 'city'], 190);
    const r = await service.signup(body);
    res.json(r);
  } catch (e) { next(e); }
}
...
async function login(req, res, next) {
  ...
  try {
    ...
    // brute-force/credential-stuffing from logs at all. identity is logged,
    // not the password.
    logger.info({ ip: req.ip, identity, requireVerification: !!r.requireVerification }, 'login: success');
    res.json(r);
  } catch (e) {
    logger.warn({ ip: req.ip, identity, code: e.code }, 'login: failed');
    next(e);
  }
}
...
async function verifyAdmin2Fa(req, res, next) {
  const { email } = req.body || {};
  try {
    const r = await service.verifyAdmin2Fa({ email, code: (req.body || {}).code });
    logger.info({ ip: req.ip, email }, 'admin login: 2FA step succeeded — session granted');
    res.json(r);
  } catch (e) {
    logger.warn({ ip: req.ip, email, code: e.code }, 'admin login: 2FA step failed');
    next(e);
  }
}
...
/**
 * Authenticated self-service password change (requires current password).
 * Used both for a normal password change and to clear mustChangePassword
 * after an admin-issued first login.
 */
async function changeOwnPassword(req, res, next) {
  try {
    const { currentPassword, newPassword } = req.body || {};
    const r = await service.changeOwnPassword({ userId: req.user.id, currentPassword, newPassword });
    res.json(r);
  } catch (e) { next(e); }
}
...
async function adminLogin(req, res, next) {
  const { email } = req.body || {};
  try {
    const r = await service.loginWithPassword({ email, role: 'admin', password: (req.body || {}).password });
