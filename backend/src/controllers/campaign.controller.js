...
/**
 * Recomputes totalAmount/baseAmount/gstAmount for a Grow Business campaign
 * from the admin-configured price list instead of trusting the client.
 * Without this, a vendor could submit an arbitrary totalAmount (e.g. ₹1),
 * pay that trivial amount via Razorpay, and receive an auto-approved
 * campaign (see activateSubscription in payment.controller.js, which sets
 * adminStatus: 'approved' on successful payment with no further checks).
 * Returns { totalAmount, baseAmount, gstAmount } or null if packageType
...
function resolvePlanPricing(packageType, planDays, countryCode) {
  ...
  const plan = pkg.plans.find((p) => !p.custom && p.days === days);
  if (!plan || !Number.isFinite(Number(plan.price))) return null;
  const totalAmount = Math.round(Number(plan.price));
...
async function createCampaign(req, res, next) {
  try {
    ...
    sanitizeFields(req.body, ['goal', 'targetCity', 'targetAudience', 'creativeCopy', 'targetAreas'], 2000);
    const {
    ...
    // Pricing must never be trusted from the client: a matching non-custom
    // plan is repriced to the admin-configured amount; a custom plan is
    // floored at that package's listed "Custom Campaign" starting price.
    // Otherwise a vendor could submit totalAmount: 1, pay ~₹1, and receive
    // an auto-approved campaign (activateSubscription approves on payment
    // success with no further amount check).
    ...
    if (packageType) {
      ...
      if (resolved) {
        ...
        finalGstAmount = finalTotalAmount - finalBaseAmount;
      }
    }
...
async function updateCampaign(req, res, next) {
  try {
    const vendor = await getVendorOrThrow(req);
    const { id } = req.params;
    sanitizeFields(req.body, ['goal', 'targetCity', 'targetAudience', 'creativeCopy'], 2000);
...
async function adminUpdateCampaign(req, res, next) {
  try {
    const { id } = req.params;
    sanitizeFields(req.body, ['adminNotes'], 2000);
    const {
