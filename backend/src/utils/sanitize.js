...
function stripDangerousTags(str) {
  ...
  return str
    ...
    // Remove <svg>/<img> tags outright — this is a blacklist tag-stripper,
    // not an HTML escaper, and both are common XSS payload carriers
    // (<svg onload=...>, <img src=x onerror=...>) that were previously left
    // completely untouched as long as the onerror/onload attribute survived.
    .replace(/<svg[^>]*>[\s\S]*?<\/svg>/gi, '')
    .replace(/<svg[^>]*\/>/gi, '')
    .replace(/<img[^>]*>/gi, '')
    // Remove event handlers (onclick, onerror, onload, etc.). Matches any
    // non-word character (space, slash, tab, newline, quote) before "on" —
    // previously required a literal space, so "<tag/onerror=...>" (XML
    // self-closing slash instead of a space) sailed straight through.
    .replace(/[^\w]on\w+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '')
