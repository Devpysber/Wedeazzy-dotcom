...
# NOTE: Docker inserts its own iptables DNAT/ACCEPT rules for any port a
# container publishes (the `ports:` list in docker-compose.yml) — these
# BYPASS ufw entirely, a well-known Docker+ufw gotcha. docker-compose.yml's
# `db` service does NOT publish 3306 to the host by default for exactly this
# reason (the `app` service reaches it over the internal compose network via
# DATABASE_URL=mysql://...@db:3306/...). Do not add a `ports: - "3306:3306"`
# mapping on a public-facing VPS — if you need external DB access for
# debugging, bind it to loopback only: `"127.0.0.1:3306:3306"`, and connect
# via an SSH tunnel instead of opening the port to the internet.
