...
async function boot() {
  ...
  // Preview Sandbox Bypass (Developer Sandbox Preview Mode) — local dev only.
  // This was reachable on the live public site by anyone (?preview=true),
  // an undocumented auth bypass into a fully "logged in" fake dashboard. It
  // never exposes real data (the couple below is hardcoded mock data), but
  // it has no reason to work outside local development.
...
async function openVendorDetailModal(vendorId, autoFocus = false) {
  ...
  }).catch((e) => console.error('Failed to log profile visit analytics event:', e));
    ...
    if (r.ok) {
      ...
      if (data) {
        ...
        if (data.guests && data.guests.length > 0) {
          state.mockData.guests = data.guests.map(g => ({
            id: g.id,
            name: g.name,
            count: g.count,
            rsvp: g.rsvp,
