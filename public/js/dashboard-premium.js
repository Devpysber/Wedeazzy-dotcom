...
async function boot() {
  ...
  // Preview Sandbox Bypass (Developer Review/Demo Mode) — local dev only.
  // This was reachable on the live public site by anyone (?preview=true), an
  // undocumented auth bypass into a fully "logged in" fake vendor dashboard.
  // It never exposes real data (mock vendor below), but has no reason to
  // work outside local development.
  const isLocalDev = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
  const isPreview = isLocalDev && (location.search.includes('preview=true') || location.search.includes('demo=true'));
  if (isPreview) {
  ...
  try {
    ...
    // Admin-issued credentials must be changed before the dashboard is usable
    // (backend also enforces this on every API call — this just avoids a
    // confusing half-loaded dashboard before the first blocked request).
    if (payload.user && payload.user.mustChangePassword) {
...
function renderDashboardTab(el) {
  ...
  if (isDemo) {
    activityHtml = `
          ...
          <!-- Timeline Item 3 -->
          <div style="position: relative;">
            <span style="position: absolute; left: -26px; top: 3px; background: #DC1F30; width: 10px; height: 10px; border-radius: 50%; border: 2px solid var(--bg-card); box-shadow: 0 0 0 3px rgba(220,31,48,0.15);"></span>
            <div>
    ...
    if (recentForActivity.length > 0) {
      activityHtml = recentForActivity.map((inq, idx) => {
        const colors = ['#2563eb', '#10b981', '#DC1F30', '#f59e0b', '#7e22ce'];
  ...
  el.innerHTML = `
    ...
    <div style="background: linear-gradient(135deg, rgba(209, 38, 83, 0.05) 0%, rgba(209, 38, 83, 0.01) 100%); 
      ...
      <div style="flex-grow: 1;">
        ...
        <h2 style="font-family: var(--serif); font-size: 26px; color: var(--navy); margin: 0 0 14px 0; font-weight: 700; display: flex; align-items: center; gap: 6px;">
          ...
          <span style="display: inline-flex; align-items: center; justify-content: center; background: #DC1F30; color: white; border-radius: 50%; width: 18px; height: 18px; font-size: 10px; font-weight: bold; vertical-align: middle;">✔</span>
        </h2>
      ...
      <div style="display: flex; align-items: center; gap: 16px; background: rgba(255, 255, 255, 0.6); border: 1px solid var(--pink-border); border-radius: 12px; padding: 14px 20px; box-shadow: var(--shadow-sm);">
        <div style="background: #FFF0F2; width: 46px; height: 46px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px; color: #DC1F30;">👑</div>
        <div>
          ...
          <span style="font-size: 11.5px; color: #DC1F30; font-weight: 700; display: block; margin-top: 2px;">Expires in 24 days</span>
        </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 20px; margin-bottom: 32px;">
      ...
      <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 20px; display: flex; align-items: center; gap: 16px; transition: all 0.3s ease; box-shadow: var(--shadow-premium);"
        ...
        <div style="background: rgba(220, 31, 48, 0.08); color: #DC1F30; width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0;">👁</div>
        <div>
      ...
      <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 20px; display: flex; align-items: center; gap: 16px; transition: all 0.3s ease; box-shadow: var(--shadow-premium);"
        ...
        <div style="flex-grow: 1;">
          ...
          <div style="height: 6px; background: var(--border-color); border-radius: 99px; overflow: hidden; width: 100%; margin-top: 10px;">
            <div style="width: ${compPercent}%; height: 100%; background: #DC1F30; border-radius: 99px;"></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card-premium" style="padding: 24px; border-radius: 16px; box-shadow: var(--shadow-premium); background: var(--bg-card);">
        <div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
          <div style="display: flex; align-items: center;">
            ...
            <span style="background: rgba(220, 31, 48, 0.08); color: #DC1F30; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 20px; margin-left: 10px;">${unreadEnquiriesCount || 5} New</span>
          </div>
          <button onclick="switchTab('leads')" style="background: none; border: none; color: #DC1F30; font-weight: 700; font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 4px;">
                  ...
                  return `
                    <tr style="border-bottom: 1px solid var(--border-color); transition: background 0.2s;" onmouseover="this.style.background='rgba(209,38,83,0.01)'" onmouseout="this.style.background='none'">
                      ...
                      <td style="padding: 14px 16px; font-size: 13px; color: var(--text-primary);">
                        ${i.eventDate || 'TBD'}
                        <span style="font-size: 11px; color: #DC1F30; font-weight: 600; display: block; margin-top: 2px;">${daysLeftVal}</span>
                      </td>
                      ...
                      <td style="padding: 14px 16px;">
                        <span class="status-badge ${statusVal}" 
                              style="font-size: 9px; font-weight: 800; padding: 4px 10px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.5px; display: inline-block;
                                     ${statusVal === 'new' ? 'background: #FFEBEF; color: #DC1F30;' :
                      ...
                      <td style="padding: 14px 16px; text-align: right;">
                        <button class="btn-premium btn-outline" style="font-size: 11.5px; padding: 6px 14px; border-radius: 6px; border: 1px solid var(--pink-border); color: #DC1F30; background: transparent; font-weight: 700; transition: all 0.2s;"
                                onmouseover="this.style.background='#DC1F30'; this.style.color='white';"
                                onmouseout="this.style.background='transparent'; this.style.color='#DC1F30';"
      ...
      <div style="display: flex; flex-direction: column; gap: 28px;">
        ...
        <div class="card-premium" style="padding: 24px; border-radius: 16px; box-shadow: var(--shadow-premium); background: var(--bg-card); border-color: ${activePlan === 'Featured' ? '#FBBF24' : activePlan === 'Premium' ? 'var(--pink-border)' : 'var(--border-color)'};">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; border-bottom: 1px solid var(--border-color); padding-bottom: 10px;">
            ...
            <span style="background:#FFEBEF; color:#DC1F30; font-size:9.5px; font-weight:800; padding:2px 8px; border-radius:6px; text-transform:uppercase;">Premium</span>
          </div>

          <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
            <div style="background: #FFF0F2; width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px; color: #DC1F30; flex-shrink: 0;">👑</div>
        ...
        <div class="card-premium" style="padding: 24px; border-radius: 16px; box-shadow: var(--shadow-premium); background: var(--bg-card);">
          ...
          <div style="height: 7px; background: var(--border-color); border-radius: 99px; overflow: hidden; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05); margin-bottom: 20px;">
            <div style="width: ${compPercent}%; height: 100%; background: linear-gradient(90deg, #DC1F30, #FFA6B2); border-radius: 99px;"></div>
          </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 28px; margin-bottom: 24px;" class="form-grid-premium">
      ...
      <div class="card-premium" style="padding: 24px; border-radius: 16px; box-shadow: var(--shadow-premium); background: var(--bg-card);">
        <div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
          ...
          <button onclick="switchTab('reports')" style="background: none; border: none; color: #DC1F30; font-weight: 700; font-size: 12px; cursor: pointer; display: flex; align-items: center; gap: 4px;">
        ...
        <div style="display: flex; gap: 16px;">
          ...
          <div style="flex:1; border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center;">
            <div style="background: rgba(220, 31, 48, 0.08); color: #DC1F30; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 14px; margin: 0 auto 10px auto;">👁</div>
      ...
      <div class="card-premium" style="padding: 24px; border-radius: 16px; box-shadow: var(--shadow-premium); background: var(--bg-card);">
        <div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
          ...
          <a href="#" onclick="switchTab('leads'); event.preventDefault();" style="color: #DC1F30; font-weight: 700; font-size: 12px; text-decoration: none;">View All</a>
        </div>
  ...
  el.innerHTML = `
    <style>
      ...
      .input-with-icon-wrapper input:focus, .input-with-icon-wrapper select:focus {
        border-color: #DC1F30 !important;
        box-shadow: 0 0 0 3px rgba(220, 31, 48, 0.08) !important;
        background-color: #FFFFFF !important;
        outline: none;
      }
      .input-icon-span {
    ...
    <div class="profile-saas-layout">
      ...
      <div style="display:flex; flex-direction:column; gap:28px;">
        ...
        <div class="card-premium" id="personalCard" style="border-radius: 12px; box-shadow: var(--shadow-premium);">
          <div class="card-header-premium" style="margin-bottom:20px; border-bottom: 1px solid var(--border-color); padding-bottom: 14px;">
            <h3 style="font-family:var(--serif); font-size:18px; color:var(--navy); margin:0; display:flex; align-items:center; gap:8px;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #DC1F30;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
              Personal Details
            </h3>
        ...
        <div class="card-premium" id="businessCard" style="border-radius: 12px; box-shadow: var(--shadow-premium);">
          <div class="card-header-premium" style="margin-bottom:20px; border-bottom: 1px solid var(--border-color); padding-bottom: 14px;">
            <h3 style="font-family:var(--serif); font-size:18px; color:var(--navy); margin:0; display:flex; align-items:center; gap:8px;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #DC1F30;"><rect x="2" y="2" width="20" height="20" rx="2" ry="2"></rect><path d="M7 21v-4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v4"></path><path d="M8 7h8"></path><path d="M8 11h8"></path></svg>
              Business Details
            </h3>
          ...
          <div class="form-grid-premium" style="gap:20px;">
            ...
            <div class="form-field-premium">
              ...
              <input type="number" id="pMin" value="${v.priceMin || ''}" placeholder="e.g. 50000" min="0" oninput="window.onProfileFieldChange();" />
            </div>
            ...
            <div class="form-field-premium" id="capacityFieldWrapper" style="${(v.category === 'Banquet Halls' || v.category === 'Marriage Gardens') ? '' : 'display:none;'}">
              ...
              <input type="number" id="pCap" value="${v.capacity || ''}" placeholder="e.g. 800" min="1" oninput="window.onProfileFieldChange();" />
            </div>
            ...
            <div>
              ...
              <div style="display:flex; flex-wrap:wrap; gap:8px;">
                ${['Candid Photography', 'Drone Coverage', 'Same Day Preview', 'Destination Weddings', 'Luxury Weddings', 'AC Banquet Hall', 'In-house Catering', 'Valet Parking', 'Bridal Suite'].map(tag => `
                  <button type="button" onclick="window.addSuggestedHighlight('${esc(tag)}')" style="border:1px solid #E2E8F0; background:#FFFFFF; padding:6px 12px; border-radius:30px; font-size:12px; font-weight:600; color:var(--text-secondary); cursor:pointer; transition:all 0.2s;" onmouseover="this.style.borderColor='var(--pink-border)'; this.style.color='#DC1F30';" onmouseout="this.style.borderColor='#E2E8F0'; this.style.color='var(--text-secondary)';">${esc(tag)}</button>
                `).join('')}
              </div>
            </div>
          </div>
        ...
        <div class="card-premium" id="galleryCard" style="border-radius: 12px; box-shadow: var(--shadow-premium);">
          <div class="card-header-premium" style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:20px; border-bottom: 1px solid var(--border-color); padding-bottom: 14px;">
            <div>
              <h3 style="font-family:var(--serif); font-size:18px; color:var(--navy); margin:0; display:flex; align-items:center; gap:8px;">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #DC1F30;"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>
          ...
          <div style="margin-bottom:20px;">
            <div style="height:6px; background-color:#F1F5F9; border-radius:99px; overflow:hidden; margin-bottom:6px;">
              <div style="width:${(currentPhotos / maxPhotos) * 100}%; height:100%; background:linear-gradient(90deg, #DC1F30, #ef4444); border-radius:99px;"></div>
            </div>
          ...
          <div class="photo-upload-zone" 
            ...
            <div style="background: rgba(209, 38, 83, 0.08); border-radius: 50%; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; margin-bottom: 12px;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#DC1F30" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="sticky-right-sidebar">
        ...
        <div class="card-premium" style="padding: 20px; border-radius: 12px; box-shadow: var(--shadow-premium);">
          ...
          <div style="height:8px; background-color:#F1F5F9; border-radius:99px; overflow:hidden; margin-bottom:16px;">
            <div style="width:${visibilityScore}%; height:100%; background:linear-gradient(90deg, #DC1F30, #ef4444); border-radius:99px;"></div>
          </div>
  ...
  el.innerHTML = `
    ...
    <div class="card-premium" style="margin-bottom: 24px; padding: 20px; border-radius: 12px; background: var(--bg-card); box-shadow: var(--shadow-sm); border: 1px solid var(--border-color);">
      ...
      <div style="height: 8px; background: var(--border-color); border-radius: 99px; overflow: hidden; width: 100%;">
        <div style="width: ${percent}%; height: 100%; background: linear-gradient(90deg, #DC1F30 0%, #ff4b5c 100%); border-radius: 99px; transition: width 0.5s ease;"></div>
      </div>
    </div>
...
function renderAddBusinessTab(el) {
  ...
  if (count >= limit) {
    el.innerHTML = `
      <div class="card-premium" style="max-width:600px; margin: 40px auto; text-align: center; padding: 40px 28px; border-radius: 16px; background: var(--bg-card); box-shadow: var(--shadow-premium); border: 1px solid var(--border-color);">
        <div style="background: rgba(220, 31, 48, 0.06); color: #DC1F30; width: 64px; height: 64px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; margin: 0 auto 20px auto;">🔒</div>
    ...
    <div class="card-premium" style="max-width:800px; margin: 0 auto;">
      ...
      <div style="margin-bottom: 24px; padding: 16px; border-radius: 8px; background: rgba(209, 38, 83, 0.02); border: 1px solid var(--pink-border);">
        ...
        <div style="height: 6px; background: var(--border-color); border-radius: 99px; overflow: hidden; width: 100%;">
          <div style="width: ${percent}%; height: 100%; background: linear-gradient(90deg, #DC1F30 0%, #ff4b5c 100%); border-radius: 99px;"></div>
        </div>
        ...
        <div class="form-grid-premium">
          ...
          <div class="form-field-premium">
            ...
            <select id="addBizCat">
              ...
              <option>Mehendi Designers</option>
              <option>Decorators &amp; Stage Lights</option>
            </select>
          </div>
          <div class="form-field-premium">
      ...
      <div class="form-step" id="step-2">
        ...
        <div class="form-grid-premium">
          <div class="form-field-premium">
            ...
            <input type="number" id="addBizCap" placeholder="500" min="1" />
          </div>
          <div class="form-field-premium">
            <label>Starting Package Price (₹)</label>
            <input type="number" id="addBizPrice" placeholder="1500" min="0" />
          </div>
          <div class="form-field-premium full">
  ...
  populateBizCategoryCityOptions();
}

// The hardcoded category/city <option> lists above are a fallback for when
// this fetch fails — refresh them from the platform's real, currently
// searchable taxonomy so a new listing's category/city always matches
// something browse.js's filters can actually find (a mismatch here made
// past listings effectively unsearchable — categorySlug/citySlug are
// derived straight from these labels on submit, see submitNewBusiness).
async function populateBizCategoryCityOptions() {
  try {
    const res = await api('/api/public/metadata');
    if (!res || !res.ok) return;
    const catSel = document.getElementById('addBizCat');
    const citySel = document.getElementById('addBizCity');
    if (catSel && Array.isArray(res.categories) && res.categories.length) {
      catSel.innerHTML = res.categories.map(c => `<option>${esc(c.name)}</option>`).join('');
    }
    if (citySel && Array.isArray(res.cities) && res.cities.length) {
      citySel.innerHTML = res.cities.map(c => `<option>${esc(c.name)}</option>`).join('');
    }
  } catch (e) {
    // Keep the static fallback options already rendered.
  }
}
...
function renderReviewsTab(el) {
  // Only an explicit ?preview=true/?demo=true visit is "demo mode" — a real
  // vendor whose state.vendor failed to load (network hiccup, race condition)
  // must NOT fall into this branch, or they'd see three fabricated reviews
  // presented as if they were real customer feedback.
  const isDemo = location.search.includes('preview=true') || location.search.includes('demo=true');
  const hasRealReviews = !!(state.vendor?.reviews && state.vendor.reviews.length > 0);

  const displayReviews = hasRealReviews
    ? state.vendor.reviews.map(r => ({
        name: r.name,
        rating: r.rating,
        text: r.text,
  ...
  el.innerHTML = `
    <div class="card-premium" style="margin-bottom: 24px;">
      ...
      <div style="background: linear-gradient(135deg, rgba(234, 179, 8, 0.08) 0%, rgba(234, 179, 8, 0.02) 100%); border: 1px solid rgba(234, 179, 8, 0.3); border-radius: 8px; padding: 16px 20px; display: flex; align-items: center; justify-content: space-between;">
        ...
        <button class="btn-premium" onclick="triggerToast('Connecting to VIP support concierge...')" style="border-color: var(--gold); color: var(--gold);">Call Support</button>
      </div>
    </div>
  `;
...
async function executeDeleteProfile() {
  ...
  try {
    ...
    triggerToast('Account deletion is not available yet — please contact support@wedeazzy.com to request removal.');
    btn.disabled = false;
    ...
    stepHtml = `
      ...
      <div class="grow-banner-pink flex justify-between items-center p-6 rounded-2xl mb-6 relative overflow-hidden" style="background: linear-gradient(135deg, #FFF0F2 0%, #FFE2E6 100%); border: 1px solid var(--pink-border);">
        ...
        <svg viewBox="0 0 100 100" class="w-20 h-20" style="flex-shrink: 0; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.05));">
          ...
          <circle cx="50" cy="50" r="25" fill="#DC1F30" />
          <circle cx="50" cy="50" r="15" fill="#FFE4E6" />
          <circle cx="50" cy="50" r="6" fill="#B3131F" />
          <!-- Arrow -->
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
            ...
            <svg viewBox="0 0 400 200" class="w-full h-full" style="display: block;">
              ...
              <g transform="translate(100, 60)">
                ...
                <g transform="translate(100, 65)">
                  <path d="M 0,0 C -8,-8 -16,-16 -16,-28 C -16,-38 -8,-46 0,-46 C 8,-46 16,-38 16,-28 C 16,-16 8,-8 0,0 Z" fill="#DC1F30" />
          ...
          <div id="wizHint" class="text-xs font-semibold" style="color: #DC1F30; display: none; margin-bottom: 8px;">
            ⚠️ Please click Add or press Enter to save this location.
          </div>
          <div id="wizValidationMsg" class="text-xs font-bold mb-4" style="color: ${wizardState.targetAreas.length > 0 ? '#10B981' : '#DC1F30'};">
        ...
        <div class="payment-summary-column">
          <div class="payment-summary-card">
            ...
            <div class="support-details-container">
              ...
              <div>Phone: <a href="tel:+919930090487">+91 99300 90487</a></div>
...
window.renderWizChipsAndValidation = function() {
  ...
  if (valMsg) {
    valMsg.style.color = wizardState.targetAreas.length > 0 ? '#10B981' : '#DC1F30';
...
window.showPaymentOverlay = function(message) {
  ...
  overlay.innerHTML = `
    <div class="payment-loader-spinner" style="width: 50px; height: 50px; border: 4px solid rgba(255, 255, 255, 0.15); border-left-color: #DC1F30; border-radius: 50%; animation: spin 1s linear infinite;"></div>
...
// Overlays admin-edited prices (backend/src/config/growCampaignsPricing.json) onto the
...
async function applyGrowCampaignsPricing(countryCode) {
  try {
    ...
    Object.keys(res.pricing).forEach((key) => {
      ...
      if (!pkg || !Array.isArray(incomingPlans)) return;
      pkg.plans.forEach((tier, idx) => {
        const incoming = incomingPlans[idx];
        if (!incoming) return;
    ...
    // Keep the hardcoded defaults already in PACKAGES.
  }
}
...
function renderGrowLanding(el) {
  ...
  el.innerHTML = `
    ...
    <div class="whatsapp-help-banner">
      ...
      <a href="https://wa.me/919930090487?text=Hello%20WedEazzy%20Support%2C%20I%20need%20help%20growing%20my%20wedding%20business." target="_blank" class="whatsapp-btn">
...
function renderGrowSettings(el) {
  ...
  el.innerHTML = `
    <div class="card-premium">
      ...
      <!-- Target Areas -->
      <span class="grow-section-label">Target Areas <span style="color:#DC1F30;">*</span> <em style="font-weight:400;text-transform:none;font-style:normal;font-size:10px;color:var(--text-muted);">(required)</em></span>
        ...
        <svg viewBox="0 0 400 120" width="100%" height="120" style="opacity:0.85; margin: 0 auto; display:block;">
          <defs>
            ...
            <radialGradient id="targetGlow" cx="50%" cy="50%" r="50%">
              <stop offset="0%" stop-color="rgba(220, 31, 48, 0.2)" />
              <stop offset="100%" stop-color="rgba(220, 31, 48, 0)" />
            </radialGradient>
          </defs>
          ...
          <circle cx="200" cy="60" r="25" fill="none" stroke="#DC1F30" stroke-dasharray="4,4" stroke-width="1.5" />
          <circle cx="200" cy="60" r="4" fill="#DC1F30" />
      ...
      <!-- Real-time Help Hint and Validation Message -->
      <div id="growHint" class="text-xs font-semibold" style="color: #DC1F30; display: none; margin-bottom: 8px; font-family: var(--sans);">
        ⚠️ Please click Add or press Enter to save this location.
      </div>
      <div id="growValidationMsg" class="text-xs font-bold mb-4" style="color: ${growState.targetAreas.length > 0 ? '#10B981' : '#DC1F30'}; font-family: var(--sans);">
      ...
      <textarea id="targetAudienceInput" 
        ...
        onfocus="this.style.borderColor='#DC1F30'"
        onblur="this.style.borderColor='var(--border-color)'"
      ...
      <div class="advanced-content" id="advancedContent">
        ...
        <div style="position:relative;margin-bottom:24px;">
          <div style="width:100%;height:6px;border-radius:99px;background:var(--border-color);position:relative;margin:10px 0;">
            <div id="ageTrackFill" style="position:absolute;height:6px;border-radius:99px;background:#DC1F30;left:${((growState.ageMin-18)/47)*100}%;right:${100-((growState.ageMax-18)/47)*100}%;"></div>
          </div>
...
window.renderGrowChipsAndValidation = function() {
  ...
  if (valMsg) {
    valMsg.style.color = growState.targetAreas.length > 0 ? '#10B981' : '#DC1F30';
      ...
      <div class="payment-success-screen" style="text-align: center; padding: 40px 20px;">
        ...
        <div style="margin-top:28px; display:flex; gap:16px; flex-wrap:wrap; justify-content:center;">
          <button onclick="growGoStep('my-campaigns')" 
            style="background:linear-gradient(135deg, #DC1F30, #C21421);color:#fff;border:none;border-radius:10px;padding:13px 28px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 15px rgba(220,31,48,0.18); transition: transform 0.2s, box-shadow 0.2s;"
            onmouseover="this.style.transform='translateY(-1px)'; this.style.boxShadow='0 6px 20px rgba(220,31,48,0.28)';"
            onmouseout="this.style.transform='none'; this.style.boxShadow='0 4px 15px rgba(220,31,48,0.18)';">
            View My Campaigns
          </button>
  ...
  try {
            ...
            <div class="premium-empty-state">
              <div class="empty-state-illust-wrap">
                <svg viewBox="0 0 200 200" width="100%" height="100%" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M70 140V90C70 84.4772 74.4772 80 80 80H120C125.523 80 130 84.4772 130 90V140" stroke="#DC1F30" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
                  <path d="M100 50V70" stroke="#DC1F30" stroke-width="4" stroke-linecap="round" />
                  <circle cx="100" cy="45" r="5" fill="#DC1F30" />
                  <rect x="85" y="105" width="30" height="20" rx="3" stroke="#DC1F30" stroke-width="4" />
                  <path d="M60 140H140" stroke="#DC1F30" stroke-width="4" stroke-linecap="round" />
                  <defs>
                    <linearGradient id="gradient-bg" x1="20" y1="20" x2="180" y2="180" gradientUnits="userSpaceOnUse">
                      <stop stop-color="#DC1F30" />
              ...
              <div class="premium-campaign-card">
                ...
                <div class="campaign-actions-row">
                  <div class="campaign-actions-left">
                    ${isUnpaid ? `
                      <button onclick="growPayForCampaign('${c.id}')" class="action-btn-text" style="color: #FFFFFF; background: #DC1F30; border-color: #DC1F30; padding: 6px 16px; border-radius: 8px; font-weight: 700; cursor: pointer; border: 1.5px solid; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 10px rgba(220,31,48,0.2);">
                        💳 Pay Now / Retry
                      </button>
            ...
            data: {
              ...
              datasets: [
                { 
                  ...
                  data: reachData, 
                  borderColor: '#DC1F30', 
                  backgroundColor: 'rgba(220,31,48,0.04)', 
                  tension: 0.4, 
                  fill: true, 
                  pointRadius: 4,
                  pointHoverRadius: 6,
    ...
    el.innerHTML = `
      <div class="card-premium" style="text-align:center;padding:48px;">
        ...
        <button onclick="growGoStep('landing')" style="background:#DC1F30;color:#fff;border:none;border-radius:10px;padding:12px 24px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;">
          ← Go Back
        </button>
      </div>
    `;
      ...
      <div class="modal-premium-content" style="background:var(--bg-card); width:100%; max-width:550px; border-radius:20px; box-shadow:var(--shadow-premium); border:1px solid var(--pink-border); overflow:hidden; transform:translateY(20px); transition:transform 0.3s ease; display:flex; flex-direction:column; max-height:90vh;">
        ...
        <div style="padding:24px; overflow-y:auto; flex:1; display:flex; flex-direction:column; gap:20px;">
          ...
          <div style="display:flex; align-items:center; gap:16px; background:var(--bg-primary); padding:16px; border-radius:12px; border:1px solid var(--border-color);">
            <div style="width:48px; height:48px; border-radius:50%; background:linear-gradient(135deg, #DC1F30 0%, var(--navy) 100%); color:white; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:16px;">
          ...
          <div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
              ...
              <div style="background:var(--bg-card); padding:12px; border-radius:10px; border:1px solid var(--border-color);">
                ...
                <strong style="font-size:13.5px; color:#DC1F30;">${inq.budget || 'N/A'}</strong>
              </div>
              <div style="background:var(--bg-card); padding:12px; border-radius:10px; border:1px solid var(--border-color);">
        ...
        <div style="padding:20px 24px; border-top:1px solid var(--border-color); display:flex; justify-content:flex-end; gap:10px; background:var(--bg-primary);">
          <button onclick="closeInquiryDetailModal()" class="btn-premium btn-outline" style="padding:8px 16px; font-size:13px; font-weight:700; border: 1px solid var(--pink-border); color: #DC1F30; background: transparent; cursor: pointer; border-radius: 6px;">Close</button>
