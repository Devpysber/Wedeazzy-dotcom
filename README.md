...
2. Edit `backend/.env` with your actual database credentials:
   ...
   OTP_DEBUG_LOG=false
   ```
   If `mysql_host` is anything other than `localhost` (i.e. any of the remote/managed
   database scenarios below), append `?ssl-mode=REQUIRED` to `DATABASE_URL` so
   traffic to the database is encrypted in transit — without it, credentials
   and query data travel in plaintext over the network.
